...
+function($) {
    ...
    Modernizr;
}(this, this.document), function(global, factory) {
...
}(), window.SETTINGS = {}, SETTINGS.IS_PRODUCTION = !1, SETTINGS.APP_VERSION = "1.0.0", 
...
!function(window, $) {
    "use strict";
    function initialize() {
        App.client = {}, App.client.isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent), 
        ES6Promise.polyfill(), Modernizr.touch && FastClick.attach(document.body), $("#audioElement").length && (SoundCloud = new App.SoundCloud($("#audioElement").get(0)));
    }
    var SoundCloud;
    $(initialize);
    ...
    var SoundCloud = function($el) {
        this.$el = $el, this.el = $el[0], this.clientId = "6f1327a14a0a8b3fe7026390efdb595e", 
        this.isLoaded = !1, this.isPaused = !0, this.track = null, this.music = Array("https://soundcloud.com/vagrantrecords/active-child-johnny-belinda", "https://soundcloud.com/thescumfrog/dirty-vegas-days-go-by-the", "https://soundcloud.com/max-richter/02-spring-1"), 
        this.setupHandlers();
    };
    SoundCloud.prototype.setupHandlers = function() {
        this._onTimeUpdateHandler = this._onTimeUpdateEvent.bind(this), this._onTimeEndedHandler = this._onTimeEndedEvent.bind(this), 
        this.onEnable();
    }, SoundCloud.prototype.onEnable = function() {
        this.$el.crossOrigin = "anonymous", this.$el.addEventListener("timeupdate", this._onTimeUpdateHandler, !1), 
        this.$el.addEventListener("ended", this._onTimeEndedHandler, !1), this.init();
    }, SoundCloud.prototype._onTimeUpdateEvent = function() {}, SoundCloud.prototype._onTimeEndedEvent = function() {
        var music = this.music, index = music.indexOf(this.track), remaining = music.splice(index, 1), random = remaining[Math.floor(Math.random() * remaining.length)];
        ...
        this.get("http://api.soundcloud.com/resolve.json?url=" + track + "&client_id=" + clientId, function(response) {
            that.track = JSON.parse(response), that.$el.src = that.track.stream_url + "?client_id=" + clientId, 
            that.$el.play();
        });
    }, SoundCloud.prototype.init = function() {
        var audioCtx = new (window.AudioContext || window.webkitAudioContext)(), audioSrc = audioCtx.createMediaElementSource(this.$el), analyser = audioCtx.createAnalyser();
        audioSrc.connect(analyser), audioSrc.connect(audioCtx.destination), this.play(this.random(), this.clientId);
        ...
        request.onreadystatechange = function() {
            4 === request.readyState && 200 === request.status && callback(request.responseText);
